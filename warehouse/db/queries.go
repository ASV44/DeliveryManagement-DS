...
const (
	...
	UpdateOrderWithId = "UPDATE orders SET %s = ? WHERE order_id = ?"
	DeleteOrderWithId = "DELETE FROM orders WHERE order_id = ?"
)

func SetUpdateColumn(query string, column string) string {
	return fmt.Sprintf(query, column)
}
